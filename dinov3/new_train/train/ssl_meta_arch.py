...
class SSLMetaArch(nn.Module):
    ...
    def __init__(self, cfg, cpu=False):
        ...
        self.cfg = cfg
        self.device_type = get_device(cpu=cpu)
        self.device = get_device(cpu=cpu)
        ...
        if 'head_blance_prototype' not in cfg.dino:
            cfg.dino.head_blance_prototype = False
        if 'use_history' not in cfg.dino:
            cfg.dino.use_history = False
        if cfg.dino.use_history:
            logger.info(f"Dino loss used history: {cfg.dino.use_history}, cache size: {cfg.dino.history_cache_size}")
            self.dino_loss = DINOLoss_skcache(
                    self.dino_out_dim,
                    use_history=True,
                    history_cache_size=cfg.dino.history_cache_size,
                    cfg=cfg,
                )
        elif cfg.dino.head_cache:
            logger.info(f"Dino loss used new cache: {cfg.dino.head_cache_size}")
            self.dino_loss = DINOLoss_skcache(
                self.dino_out_dim,
                student_temp=0.1,
                center_momentum=0.9,
                use_sinkhorn_queue=True,
                sk_cache=cfg.dino.head_cache_size,
            )
        elif cfg.dino.head_blance_prototype:
            logger.info(f"Dino loss used blance prototype: {cfg.dino.head_blance_prototype}")
            self.dino_loss = DINOLoss_skcache(
                self.dino_out_dim,
                student_temp=0.1,
                center_momentum=0.9,
        ...
        if "use_history" not in cfg.ibot:
            cfg.ibot.use_history = False
        if cfg.ibot.use_history:
            logger.info(f"OPTIONS -- IBOT -- use_history: {cfg.ibot.use_history}")
            logger.info(f"OPTIONS -- IBOT -- history_cache_size: {cfg.ibot.history_cache_size}")
            self.ibot_patch_loss = iBOTPatchLoss(
                cfg.ibot.head_n_prototypes,
                use_history=True,
                history_cache_size=cfg.ibot.history_cache_size,
            )
        else:
            self.ibot_patch_loss = iBOTPatchLoss(
                cfg.ibot.head_n_prototypes,
    ...
    def forward_backward(
        self, data, *, teacher_temp, iteration=0, return_cls=False, logger_freq=0, **ignored_kwargs
        ...
        global_crops = data["collated_global_crops"].to(self.device, non_blocking=True)
        local_crops = data["collated_local_crops"].to(self.device, non_blocking=True)
        masks = data["collated_masks"].to(self.device, non_blocking=True)
        mask_indices_list = data["mask_indices_list"].to(self.device, non_blocking=True)
        masks_weight = data["masks_weight"].to(self.device, non_blocking=True)
        n_masked_patches_tensor = data["n_masked_patches"].to(self.device, non_blocking=True)
        ...
        if self.has_gram_teacher:
            ...
            gram_teacher_crops = data["collated_gram_teacher_crops"].to(self.device, non_blocking=True)
        else:
        ...
        # return (loss_accumulator, metrics_dict | loss_dict, cls_dict）  if return_cls else （loss_accumulator, metrics_dict | loss_dict）
    ...
    def compute_losses(
        ...
        *,
        ...
        if logger_freq and iteration % logger_freq == 0:
            logger.info("DINO global loss: student(local crops) vs. teacher(global crops)")
        dino_local_crops_loss = self.dino_loss(
            student_logits=student_local["cls_after_head"],
            teacher_probs=teacher_global["cls_centered"],
            iteration=iteration,
            logger_freq=logger_freq,
            logger_loss="dino_local_crops_loss",
        )
        ...
        if logger_freq and iteration % logger_freq == 0:
            logger.info("DINO global loss: cls_after_head vs. cls_centered")
        dino_global_crops_loss = self.dino_loss(
            ...
            iteration=iteration,
            logger_freq=logger_freq,
            logger_loss="dino_global_crops_loss",
        )
        ...
        # IBOT loss
        if logger_freq and iteration % logger_freq == 0:
            logger.info("DINO IBOT loss: masked_patch_after_head vs. masked_patch_centered")
