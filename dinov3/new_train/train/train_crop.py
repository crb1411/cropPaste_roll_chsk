...
from dinov3.configs import setup_config, setup_job, setup_multidistillation, apply_scaling_rules_to_cfg
...
from dinov3.train import SSLMetaArch, SSLAugmentedMetaArchV2
...
def main(argv=None):
    ...
    from dinov3.new_train.utils import creat_subdir
    base_output = '/data/work/git_proj/train_out_dinov3'
    output_dir = creat_subdir(base_output, prefix='logs', create=True, time=True)
    setup_job(output_dir=output_dir, seed=0)
    cfg = apply_scaling_rules_to_cfg(cfg)
    logger.info(cfg)
    setup_logging(
        output=os.path.join(os.path.abspath(output_dir), "nan_logs"),
        name="nan_logger",
    )
