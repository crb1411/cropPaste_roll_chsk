...
def build_data_loader_from_cfg_wsi(
    ...
    try:
        ...
        dataset = WsiPatchDataset(
            tensor_transform=model.build_data_augmentation_dino(cfg),
        ...
        data_loader = make_data_loader_wsi(
            dataset=dataset,
            ...
            sampler_type=sampler_type,
            sampler_advance=start_iter*dataloader_batch_size_per_gpu,  # TODO(qas): fix this -- start_iter * cfg.train.batch_size_per_gpu,
            drop_last=True,
            chunked=True,
            collate_fn=collate_fn,
            chunk_size=32,
...
def build_multi_resolution_data_loader_from_cfg(
    ...
    for increment, (global_crops_size_i, local_crops_size_i, gram_teacher_crops_size_i) in enumerate(
        ...
        loaders.append(build_data_loader_from_cfg_wsi(cfg=cfg_i, model=model, start_iter=start_iter))
...
def main(argv=None):
    ...
    if args.output_dir is None:
        try:
            from ..new_train.utils.log_create import creat_subdir
            args.output_dir = creat_subdir(base_dir='/mnt/data/train/crb/train_out/train_svs', create=True, time=True)
        except:
            raise ImportError
